...
class AnimalStorageSession implements AnimalStorage {
	...
	protected $db;
	/** C'est une constante qui définit la clé utilisée pour stocker la base de données dans la variable superglobale $_SESSION.*/
	...
	public function __construct() {
		...
		/**session_status() vérifie si une session PHP est active. Si ce n’est pas le cas, une exception est levée. */
		if (session_status() !== PHP_SESSION_ACTIVE) {
			throw new Exception('Creating an instance of AnimalStorageSession requires an active session.');
		}
		/**
		* Si la clé SESSION_KEY existe dans la session :
		*Cela signifie qu'une base de données a déjà été créée dans cette session.
	    *Elle est chargée depuis $_SESSION[self::SESSION_KEY] dans la propriété locale $db.
		*/
		if (key_exists(self::SESSION_KEY, $_SESSION)) {
			$this->db = $_SESSION[self::SESSION_KEY];
		} else {
			/**Une nouvelle base est initialisée à partir de AnimalStorageStub, 
			 * une classe de démarrage qui fournit une liste d'animaux prédéfinis. */
